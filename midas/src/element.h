...
class Element {
 public:
  Element(int id) {
    sprite_ = std::make_shared<Sprite>(static_cast<SpriteID>(id));
  }
  ...
  bool operator==(const SpriteID& id) const {
    return sprite_->id_ == id;
  }

  bool operator==(const Element& e) const {
    return sprite_->id_ == e.id();
  }
  ...
  operator SpriteID() { return sprite_->id_; }
  SpriteID id() const { return sprite_->id_; }
  bool IsEmpty() const { return sprite_->id_ == SpriteID::Empty; }
  ...
  void Render(SDL_Renderer *renderer, int x, int y, bool render_always = false) {
    if (!render_always && (!is_visible || sprite_->id_ == SpriteID::Empty)) {
      return;
    }
    ...
    if (is_selected_) {
      ...
      SDL_RenderCopy(renderer, sprite_->sprite_, nullptr, &rc);
    }
  }
